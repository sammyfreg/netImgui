...
// - (note: before v1.91.X (2024/12/XX) ImTextureUserID was called ImTextureID)
#ifdef ImTextureID
#error Please define ImTextureUserID instead of ImTextureID!
#endif
...
// If you wrap the library from another language than C++: the functions taking ImTextureID would better take ImTextureUserID in your language binding.
struct ImTextureID
{
...
enum ImGuiBackendFlags_
{
    ...
    ImGuiBackendFlags_RendererHasTexUpdates = 1 << 4,   // Backend Renderer supports ImGuiIO::TextureRequests[] during render. This enables incremental texture updates and texture reloads.
};
...
#define IM_DRAWLIST_TEX_LINES_WIDTH_MAX     (62)
#endif
...
struct ImDrawCmd
{
    ...
    inline ImTextureUserID GetTexID() const;
};
...
enum ImTextureStatus
{
    ...
    ImTextureStatus_WantUpdates,    // Requesting backend to update specific blocks of pixels (write to texture portions which have never been used before)
    ImTextureStatus_WantDestroy,    // Requesting backend to destroy the texture
    ImTextureStatus_Destroyed,      // Backend acknowledge as destroyed.
};

// When a texture is in ImTextureStatus_WantUpdates state, we provide a list of individual rectangles to copy to GPU texture.
// You may use ImTextureData::Updates[] for the list, or ImTextureData::UpdatesMerged[] for a single bounding box.
struct ImTextureDataUpdate
{
    int X0, Y0;         // Upper-left coordinates of rectangle to update (inclusive)
    int X1, Y1;         // Lower-right coordinates of rectangle to update (exclusive)
};
...
struct IMGUI_API ImTextureData
{
    ...
    // [Internal]
    int                 UniqueID;               // Sequential index. Solely to facilitate identifying a texture when debugging/logging. Only unique per atlas.
...
struct ImFontAtlas
{
    ...
    // - If your backend supports ImGuiBackendFlags_RendererHasTexUpdates (since 1.91.6):
    ...
    // - If your backend does NOT supports ImGuiBackendFlags_RendererHasTexUpdates (older than 1.91.6):
    ...
    const ImFontBackendIO*      FontBackendIO;      // Opaque interface to a font builder (default to stb_truetype, can be changed to use FreeType by defining IMGUI_ENABLE_FREETYPE). Don't set directly!
    void*                       FontBackendData;    // Opaque data for font backend
    const char*                 FontBackendName;    // Name of font backend for display e.g. in About box
    ...
    ImVector<ImTextureData*>    TexList;            // Texture list (most often TexList.Size == 1)
    ImFontAtlasBuilder*         Builder;            // [Internal] Opaque
    ImDrawListSharedData*       DrawListSharedData; // [Internal] In principle this could become an array (e.g. multiple contexts using same atlas)
...
struct ImFont
{
    ...
    bool                        _LockDisableLoading;
    const ImFontConfig*         _LockSingleSrcConfig;
    ...
    IMGUI_API float             GetCharAdvance(ImWchar c);          // { return ((int)c < IndexAdvanceX.Size) ? IndexAdvanceX[(int)c] : FallbackAdvanceX; }
    ...
    IMGUI_API void              BuildRegisterGlyph(const ImFontConfig* font_cfg, const ImFontGlyph* glyph);
...
inline ImTextureUserID ImDrawCmd::GetTexID() const
{
    ImTextureUserID tex_id = TextureId._TexData ? TextureId._TexData->BackendTexID : TextureId._TexUserID;
...
struct ImGuiPlatformIO
{
    ...
    // Maximum texture size supported by renderer (used to adjust how we size textures). 0 if not known.
